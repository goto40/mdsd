...
local point = require("big_example.Point")
local polygon = require("big_example.Polygon")

manual_protocol.fields = {}
for _,f in ipairs(polygon.fields) do table.insert(manual_protocol.fields, f) end
for _,f in ipairs(point.fields) do table.insert(manual_protocol.fields, f) end

function manual_protocol.dissector(buffer, pinfo, tree)
  pinfo.cols.protocol = manual_protocol.name
  polygon.dissector_data(manual_protocol, buffer, 0, tree)
end
