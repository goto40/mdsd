...
def get_referenced_elements_of_struct(i):
    ...
    res = filter(
        lambda x: textx_isinstance(x, mm["Enum"])
        or textx_isinstance(x, mm["Struct"])
        or textx_isinstance(x, mm["Constants"]),
        res,
    )
...
def compute_formula_for_internaltype(f, internaltype, info="formula"):
    ...
    if internaltype == "INT":
        ...
        textx_assert(
            type(f.compute_formula()) is int, f, info + " must be an INT/BOOL as int"
        )
        textx_assert(
            f.compute_formula() == 0 or f.compute_formula() == 1,
            f,
    ...
    if textx.textx_isinstance(attr, textx.get_metamodel(attr)["ScalarAttribute"]):
        ...
        if attr.has_fixed_size():
            ...
            raise TextXSemanticError(
                "unexpected: no fixed dimension available for {}".format(attr.name),
                **get_location(attr)
            )
...
def _assert_is_embedded(atype):
    if not atype.is_embedded():
        raise TextXSemanticError(
            "unexpected: get_start_end_bit called for an non-embedded attribute {}".format(
                atype
            ),
            **get_location(atype)
        )
...
def get_container(atype):
    ...
    if p is None:
        raise TextXSemanticError(
            "unexpected: did not found container of embedded attribute {}".format(
                atype
            ),
            **get_location(atype)
        )
...
def get_start_end_bit(myattr):
    ...
    textx_assert(
        idx >= 0,
        myattr,
        "unexpected: element not in list of embedded elements of own conatainer",
    )
    lst = lst[0:idx]
    ...
    textx_assert(
        start_bit >= 0 and start_bit < total_bits,
        myattr,
        "plausibility check bits={}..{}".format(start_bit, end_bit),
    )
    textx_assert(
        end_bit >= 0 and end_bit < total_bits,
        myattr,
        "plausibility check bits={}..{}".format(start_bit, end_bit),
    )
    textx_assert(
...
def get_bits(x):
    ...
    if textx.textx_isinstance(x, mm["RawType"]):
        ...
        raise TextXSemanticError(
            "unexpected: no bits available for {}".format(x.name), **get_location(x)
        )
