...
from textx import (get_metamodel, TextXSemanticError, textx_isinstance, get_location,
                   get_model)
from item_lang.common import (textx_assert, compute_formula_for_internaltype, find_builtin)
...
def get_property(attr, prop_name):
    def get_value(res, internaltype):
        if internaltype == 'STRING':
            textx_assert(res.textValue is not None, attr, prop_name + " must be a STRING")
            return res.textValue.x
        elif internaltype == 'INT':
            textx_assert(res.numberValue is not None, attr, prop_name + " must be an NUMBER/INT")
        elif internaltype == 'UINT':
            textx_assert(res.numberValue is not None, attr, prop_name + " must be an NUMBER/UINT")
        elif internaltype == 'BOOL':
            textx_assert(res.numberValue is not None, attr, prop_name + " must be an NUMBER/BOOL as int")
        elif internaltype == 'FLOAT':
            textx_assert(res.numberValue is not None, attr, prop_name + " must be an NUMBER")
            assert res.numberValue is not None
        elif internaltype == 'ENUM':
            textx_assert(res.numberValue is not None, attr, prop_name + " must be an ENUM")
            assert res.numberValue is not None
        else:
            return None
    ...
    if len(res) == 0:
        ...
        if internaltype == 'ATTRTYPE':
            ...
            correct_instance = textx_isinstance(
                ...
                attr.type, mm['Enum'])
            textx_assert(correct_instance, attr, prop_name + " only applicable for rawtypes/enums")
            internaltype = attr.type.internaltype
        value = get_value(res, internaltype)
        textx_assert(value is not None, attr, prop_name + " could not be interpreted (unexpected)")
        return value
...
def get_default_property_set(mm):
    ...
    for m in mm.builtin_models:
        res = find(m,
                   "built_in.default_properties", "+m:package.property_sets",
...
def get_property_set(model_obj):
    res = find(model_obj, "", "+m:^~property_set",
...
def get_all_possible_properties(model_obj, filter_applicable_to_model_object=True):
    ...
    def get(ps):
        ...
        if ps.extends is not None:
            ...
            res = res | set(get_default_property_set(
                get_metamodel(model_obj)).property_definitions)
        return res
...
def get_property_type(attr, prop_name):
    def get_type(internaltype):
        if internaltype == 'STRING':
            ...
            return find_builtin(attr, "built_in.int32", "+m:^package.items")  # +m ist not required..
        elif internaltype == 'UINT':
...
def has_property(attr, prop_name):
    ...
    if len(res) == 0:
        if prop_name not in get_all_possible_properties(attr):
            raise TextXSemanticError("{} not a possible property".format(prop_name), **get_location(attr))
        return False
    else:
        assert len(res) == 1
        return True
...
def is_applicable_for(parent, definition):
    ...
    if len(appl) == 0:
        ...
        if textx_isinstance(parent, mm["Struct"]) and definition.internaltype=='ATTRTYPE':
            return False
        return True
    ...
    if textx_isinstance(attr, mm["VariantAttribute"]) and 'variant' in appl:
        return True
    if textx_isinstance(attr, mm["ArrayAttribute"]) or textx_isinstance(attr, mm["ScalarAttribute"]):
        ...
        if textx_isinstance(attr.type, mm["RawType"]):
            rt_appl = list(filter(lambda x: textx_isinstance(x, mm['ApplicableForRawType']), appl))
            allowed = list(reduce(lambda x, y: x.extend(y), map(lambda x: x.concrete_types, rt_appl)))
