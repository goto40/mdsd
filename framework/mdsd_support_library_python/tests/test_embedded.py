...
def test_uint_from():
    ...
    for x in range(0, 8):
        assert x>=0 and x<8
        c = set_embedded_in_uint(np.uint8(x), c,[60,59])
        assert isinstance(c, np.uint64)
        get_embedded_from_uint(np.uint8, c, [60,59]) == np.uint8(x)
    assert isinstance(c, np.uint64)
    assert c == np.bitwise_not(np.uint64(0x0))

    c = set_embedded_in_uint(np.uint8(8), c, [60, 59])
    assert isinstance(c, np.uint64)
    get_embedded_from_uint(np.uint8, c, [60, 59]) != np.uint8(8)

    # 2 bits: -4..3
    c = np.bitwise_not(np.uint64(0x0))
    for x in range(-4,4):
        assert x>=-4 and x<4
        c = set_embedded_in_uint(np.int8(x), c,[60,59])
        assert isinstance(c, np.uint64)
        get_embedded_from_uint(np.int8, c, [60,59]) == np.int8(x)

    c = set_embedded_in_uint(np.int8(-1), c,[60,59])
    assert c == np.bitwise_not(np.uint64(0x0))

    c = np.uint64(0x0)
    c = set_embedded_in_uint(np.int8(-1), c,[60,59])
    c = set_embedded_in_uint(np.int8(0), c,[60,59])
