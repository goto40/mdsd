...
def generate_lua_struct(f, i):
    ...
    for a in i.attributes:
        ...
        if a.is_variant():
            ...
            concrete_variables = concrete_variables + l
    concrete_variable_names = list(
        map(
            lambda x: ".".join(map(lambda x: x.name, x.ref._tx_path)),
            concrete_variables,
        )
    )
    ...
    for a in all_fields:
        if a in fields:
            f.write(
                f'  f.field_{a.name} = ProtoField.{fqn(a.type)}("{a.name}","{a.name}", base.DEC)\n'
            )
        elif a.is_embedded():
            if a.is_scalar():
                start_end_bit = get_start_end_bit(a)
                mask = ((1 << (start_end_bit[0] + 1)) - 1) - (
                    (1 << (start_end_bit[1])) - 1
                )
                f.write(
                ...
                for k in range(n):
                    start_end_bit = (
                        start_end_bit0[0] - delta * k,
                        start_end_bit0[0] + 1 - delta * (k + 1),
                    )
                    mask = ((1 << (start_end_bit[0] + 1)) - 1) - (
                        (1 << (start_end_bit[1])) - 1
                    )
    ...
    f.write(
        "function m.dissector_data(proto, buffer, pos, tree, all_fields, var_of_interrest)\n"
    )
    ...
    for a in i.attributes:
        ...
        if a in fields:
            ...
            f.write(
                f"  sel = concrete_vars[\"{'.'.join(map(lambda x:x.name, a.variant_selector.ref._tx_path))}\"]\n"
            )
            if_text = "if"
            for m in a.mappings:
