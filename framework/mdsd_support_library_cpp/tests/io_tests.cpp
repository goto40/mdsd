...
TEST_CASE( "file_io_AllInOne_demo", "[io_tests]" ) {
  ...
  CHECK( obj.header.length == mdsd::count_bytes(obj) );
  CHECK( MDSD_get_Polygon_from_AllInOne_payload(obj).n == 4 ); // TODO: add namespace?
  ...
  CHECK( big_example::AllInOne::META::headers::__get_dim(obj, 0) == 2 );
  CHECK( big_example::AllInOne::META::headers::__get_dim(obj, 1) == 3 );
}
