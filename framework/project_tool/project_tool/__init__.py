...
class Header:
    ...
    def analyze(self, tool):
        ...
        for m in re.finditer(r'^\s*#\s*include\s*(?:"([^"]+)"|<([^>]+)>)',cpp_code, re.MULTILINE):
            try:
...
class SwigTool:
    ...
    def lookup(self, path, header=None):
        if header is not None:
            ...
            tmp = filter(lambda h:h.basedir==header.basedir and h.path.startswith(mypath), self.headers)
    ...
    def get_sorted(self):
        ...
        while len(input)>0:
            tmp = set(filter( lambda h: len(set(filter(lambda i:i in input, h.headers))) == 0, input))
            if len(tmp) == 0:
